        ...
        },
        {
            ...
            async execute(client, message: Message | PartialMessage) {
                ...
                if (channel?.isTextBased()) {
                    ...
                    await sendEmbed(channel, "🗑️ Message supprimé", `Auteur: <@${message.author?.id}>`, [
...
async function sendEmbed(channel: BaseChannel, title: string, message: string, fields: EmbedField[], color: number) {
